...
namespace helpers::mon {
    ...
    HMONITOR FindAdjacentMonitorX(HWND hwnd, bool toRight) {
        ...
        if (!GetMonitorInfoW(cur, &cmi)) return nullptr;
        ...
        for (const auto& m : mons) {
            if (m.mon == cur) continue;                 // do not consider current monitor
            long dx = m.center.x - cx;

            if (toRight) {
                if (dx <= 0) continue;                  // must be strictly to the right
                if (dx < bestDist) { bestDist = dx; best = m.mon; }
            }
            else {
                if (dx >= 0) continue;                  // must be strictly to the left
                long pdx = -dx;                         // distance magnitude
                if (pdx < bestDist) { bestDist = pdx; best = m.mon; }
            }
        }
