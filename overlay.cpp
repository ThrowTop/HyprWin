...
bool OverlayWindow::Init(HINSTANCE hInstance) {
    ...
    WNDCLASSEXW wc = {
        ...
        DefWindowProcW, 0, 0, hInstance, nullptr, nullptr, nullptr, nullptr,
        kOverlayClassName, nullptr
    };
    RegisterClassExW(&wc);

    hwnd = CreateWindowExW(
        WS_EX_TRANSPARENT | WS_EX_NOACTIVATE | WS_EX_TOPMOST, // NO WS_EX_LAYERED
        kOverlayClassName, nullptr,
        WS_POPUP,
        0, 0, 1, 1,
...
void OverlayWindow::SetColor(const D2D1_COLOR_F& color) {
    ...
    renderTarget->CreateSolidColorBrush(
        D2D1::ColorF(color.r, color.g, color.b, 0.5f),
        &fadeBrush
    );
}
...
void OverlayWindow::CreateGradientBrushes() {
    ...
    D2D1_GRADIENT_STOP stopsOuter[2] = {
        { 0.0f, D2D1::ColorF(gradientStart.r, gradientStart.g, gradientStart.b, 0.5f) },
        { 1.0f, D2D1::ColorF(gradientEnd.r, gradientEnd.g, gradientEnd.b, 0.5f) }
    };

    D2D1_GRADIENT_STOP stopsInner[2] = {
        { 0.0f, D2D1::ColorF(gradientStart.r, gradientStart.g, gradientStart.b, 1.0f) },
        { 1.0f, D2D1::ColorF(gradientEnd.r, gradientEnd.g, gradientEnd.b, 1.0f) }
    };

    if (FAILED(renderTarget->CreateGradientStopCollection(stopsOuter, 2, &gradientStopsOuter))) return;
    if (FAILED(renderTarget->CreateGradientStopCollection(stopsInner, 2, &gradientStopsInner))) return;
...
void OverlayWindow::Render() {
    ...
    if (gradient && rotating) {
        ...
        if (gradientAngleDeg > 360.0f) gradientAngleDeg -= 360.0f;
