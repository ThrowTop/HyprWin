...
class Logger {
  public:
    ...
    void set_console_level(Level lvl) {
        console_level_.store(static_cast<int>(lvl), std::memory_order_relaxed);
    }
    void set_file_level(Level lvl) {
        file_level_.store(static_cast<int>(lvl), std::memory_order_relaxed);
    }
    ...
    void logf(Level lvl, std::string_view fmt, Args&&... args) {
        ...
        if constexpr (sizeof...(Args) == 0) {
            ...
            m.text = std::apply([&](auto&... lrefs) { return std::vformat(fmt, std::make_format_args(lrefs...)); }, storage);
        }
  ...
  private:
    ...
    WORD get_color_for_level(Level lvl) const {
        switch (lvl) {
            case Level::Trace:
                return FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE; // White/Grey
            case Level::Debug:
    ...
    std::string make_prefix(const Msg& m) {
        ...
        GetDateFormatEx(
          LOCALE_NAME_USER_DEFAULT, 0, &st, opts_.date_format.empty() ? L"yyyy'-'MM'-'dd" : opts_.date_format.c_str(), dateBuf, static_cast<int>(std::size(dateBuf)), nullptr);

        GetTimeFormatEx(LOCALE_NAME_USER_DEFAULT, 0, &st, opts_.time_format.empty() ? L"HH':'mm':'ss" : opts_.time_format.c_str(), timeBuf, static_cast<int>(std::size(timeBuf)));
        ...
        oss << dt << msbuf << " [" << level_tag(m.level) << "] "
            << "[tid " << m.tid << "] ";
        return oss.str();
    }
...
inline void init(const Options& opts) {
    Logger::instance().init(opts);
}
...
inline void set_console_level(Level lvl) {
    Logger::instance().set_console_level(lvl);
}
inline void set_file_level(Level lvl) {
    Logger::instance().set_file_level(lvl);
}
