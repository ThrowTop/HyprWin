...
static bool IsShellProtected(HWND h) {
    ...
    if (GetClassNameW(h, cls, 64)) {
        if (lstrcmpW(cls, L"Progman") == 0)
            return true; // legacy desktop host
        if (lstrcmpW(cls, L"WorkerW") == 0)
            return true; // wallpaper hosts
        if (lstrcmpW(cls, L"SHELLDLL_DefView") == 0)
            return true; // desktop view
        if (lstrcmpW(cls, L"Shell_TrayWnd") == 0)
            return true; // taskbar primary
        if (lstrcmpW(cls, L"Shell_SecondaryTrayWnd") == 0)
            return true; // taskbar secondary
    }
...
static bool IsLikelyExclusiveFullscreen(HWND hwnd) {
    ...
    const bool borderless_popup = (style & WS_POPUP) && !(style & WS_CAPTION) && !(style & WS_THICKFRAME);
...
HWND FilteredTopLevel(HWND hwnd) {
    ...
    if (IsLikelyExclusiveFullscreen(top))
        return nullptr;
...
static inline bool ContainsPointVisual(HWND hwnd, const POINT& pt) {
    RECT r{};
    return utils::dwm::GetWindowVisualRect(hwnd, r) && PtInRect(&r, pt);
}
...
void SetBorderedWindow(HWND hwnd, int borderPx) {
    ...
    HMONITOR mon = mon::GetMonitorFromCursor();
    MONITORINFO mi{sizeof(mi)};
    if (!GetMonitorInfoW(mon, &mi))
        return;
    ...
    utils::dwm::SetWindowVisualRect(hwnd, vr, SWP_NOZORDER | SWP_NOOWNERZORDER | SWP_SHOWWINDOW);
}
} // namespace utils