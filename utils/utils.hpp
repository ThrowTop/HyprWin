...
#define SET_THREAD_NAME(name)                                                                                                                                                      \
    ...
    LOG_D("Thread {} ({}) | {}", GetCurrentThreadId(), GetThreadName(), __FUNCTION__)
#else
...
static inline bool SafeSendMessageTimeoutW(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp, DWORD timeout_ms, LRESULT* out_opt) {
    if (!IsWindow(hwnd))
        return false;
    ULONG_PTR r = 0;
...
inline void logWindowData(HWND hwnd) {
    ...
    LOG_T("Hw: (0x{:X}) CLS: {}, Rect: {}", reinterpret_cast<size_t>(hwnd), tinylog::WideToUtf8(std::wstring(className)), parse::rectToStr(rect));
}
